...
- **Apify free-tier budget** (`c7e4226`, `7d693c4`): The Upwork Scraper actor (`blackfalcondata/upwork-scraper`) is pay-per-event (~$0.001/run + $1 per 1,000 results) drawn from the $5/month free tier, so a hard daily query cap keeps the month inside budget. `src/lib/apifyBudget.ts` persists a UTC-day counter in SystemKv (default **16 query-runs/day**, env-overridable via `APIFY_DAILY_QUERY_BUDGET`); sync runs first in the cron so new-job ingestion gets priority and the refresh shares the same pool, skipping when empty (a budget skip is never treated as a provider failure). Query list consolidated 6→4 (`"growth manager"` replaces the client/telehealth/ecommerce variants — wider recall per billed call), sync maxResults 12→8, refresh maxResults 40→12 / cap 240→60 (was the biggest single cost), peak cooldown 20→45 min, refresh cooldown 10→45 min. Worst-case cost ≈ $0.14/day ≈ ~$4.3/month. Budget visibility exposed on `/api/sync/status` (`apifyBudget: { daily, remaining }`); schedule label now derives from the real intervals. Quality preserved — hard filters, scoring, dedup unchanged; we fetch fewer, broader queries rather than less per query.
